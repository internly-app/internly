...
interface ReviewsPageClientProps {
  ...
  initialWorkStyleFilter?: string;
  initialSortBy?: "likes" | "recent";
  initialPage?: number;
}
...
export function ReviewsPageClient({
  ...
  const [companyFilter, setCompanyFilter] = useState(
    searchParams.get("company") || initialCompanyFilter
  );
  const [workStyleFilter, setWorkStyleFilter] = useState(
    searchParams.get("work_style") || initialWorkStyleFilter
  );
  ...
  // Reviews state (sync with initialReviews prop)
  ...
  // Sync reviews state when initialReviews changes (e.g., after server revalidation)
  useEffect(() => {
  ...
  useEffect(() => {
    if (optimisticReview && reviews.some((r) => r.id === optimisticReview.id)) {
  ...
  const filteredByFilters = useMemo(() => {
    ...
    if (companyFilter) {
      filtered = filtered.filter((r) => r.company_id === companyFilter);
    }

    if (workStyleFilter) {
      filtered = filtered.filter((r) => r.work_style === workStyleFilter);
    }
    ...
    return filtered;
  }, [reviewsWithOptimistic, companyFilter, workStyleFilter, sortBy]);
